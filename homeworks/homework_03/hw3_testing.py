...
class MockOrdinaryFileWorker(OrdinaryFileWorker):
    ...
    def transfer_to_remote(self, filename):
        with open("./homeworks/homework_03/test_dir/" + filename, "r") as f_from:
            with open("./tmpf/" + filename + ".tmp", "w") as f_to:
                f_to.writelines(f_from.read())

    def transfer_to_local(self, filename):
        with open("./homeworks/homework_03/test_dir/" + filename + ".tmp", "r") as f_from:
            with open("./tmpf/" + filename, "w") as f_to:
                f_to.writelines(f_from.read())
